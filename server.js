...
app.delete(`employees/deleteLowest`, async (req, res) => {
    await db.none('DELETE FROM employees WHERE id = SELECT id FROM employees ORDER BY performance_rating ACS LIMIT 1')
    res.send('lowest employee deleted')
});
...
app.get(`/employees/highestInIt`, async (req, res) => {
   const highestInIt = await db.one('SELECT MAX(performance_rating) WHERE employees WHERE department = "IT"')
    res.send(highestInIt)
});
...
app.put(`/employees/updatePerformanceAfter2015`, async (req, res) => {
    await db.none('UPDATE employees SET performance_rating + 1 WHERE hire_date > "2015-01-01"')
    res.send('updated performance')
})
